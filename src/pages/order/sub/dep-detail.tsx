...
function DepartmentDetail() {
  const [dep, setDep] = useState<DoctorType>()
  ...
  const describeText = (): string => {
    const describe = dep?.describe ?? '暂无'
  ...
  return <View style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100%', padding: '10px' }}>
    <View className='detail_header'>
      <Image className='detail_avatar' round src={dep?.avatar ?? ''} ></Image>
      <View className='detail_label'>
        <View>
          <Text style={{ color: '#374151', fontWeight: 600, fontSize: '18px', margin: '0 5px' }}>{dep?.name ?? ''}</Text>
        </View>
        <View style={{ color: '#8B8B8B', fontWeight: 400, fontSize: '12px', margin: '5px' }}>{dep?.hospitalName ?? ''}</View>
      </View>
    </View>
